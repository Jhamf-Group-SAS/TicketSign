...
const TaskForm = ({ onCancel, onSave, initialData }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.title) {
            setToast({ message: 'El t├¡tulo es obligatorio', type: 'error' });
            return;
        }
        ...
        try {
            ...
            // L├│gica de Recurrencia (Creaci├│n M├║ltiple)
            if (!isEditing && formData.recurrence && formData.recurrence !== 'NINGUNA') {
                if (!formData.start_date || !formData.scheduled_at) {
                    setToast({ message: 'Debe indicar fecha inicio y fecha l├¡mite para la recurrencia', type: 'error' });
                    return;
                }

                const startDate = new Date(formData.start_date);
                const endDate = new Date(formData.scheduled_at);
                endDate.setHours(23, 59, 59, 999); // Incluir el ├║ltimo d├¡a completo

                if (startDate > endDate) {
                    setToast({ message: 'La fecha de inicio no puede ser posterior a la fecha l├¡mite', type: 'error' });
                    return;
                }
                ...
                // Evitar bucles infinitos o demasiadas tareas (l├¡mite seguridad 365)
                let safetyCounter = 0;
                ...
                if (taskDates.length === 0) {
                    setToast({ message: 'El rango de fechas no gener├│ ninguna tarea', type: 'error' });
                    return;
                }
                ...
                const creationPromises = taskDates.map(async (date) => {
                    ...
                    const taskData = {
                        ...
                        // Opcional: podr├¡as agregar un groupId para relacionarlas futuro
                    };
                ...
                setToast({ message: `${createdCount} tareas peri├│dicas creadas correctamente`, type: 'success' });

            } else {
                // L├│gica Original (Tarea ├Ünica o Edici├│n)
                const finalData = {
                    ...formData,
                    updatedAt: timestamp
                };
                ...
                if (!isEditing) {
                    ...
                    // Si es creaci├│n simple sin recurrencia, scheduled_at es la fecha ├║nica
                }
                ...
                if (isEditing) {
                    ...
                    // Creaci├│n Simple (Sin Recurrencia)
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 md:p-8 bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2rem] md:rounded-3xl border border-slate-200 dark:border-white/10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[92vh] md:max-h-[82vh]">
                <header className="p-5 md:p-6 border-b border-slate-100 dark:border-white/5 flex justify-between items-center bg-white/50 dark:bg-slate-900/50 backdrop-blur-md sticky top-0 z-10 shrink-0">
                    <div>
                        ...
                        <p className="text-[9px] md:text-[10px] font-medium text-slate-500 uppercase tracking-wider">Programaci├│n de servicios TI</p>
                    </div>
                ...
                <form id="task-form" onSubmit={handleSubmit} className="p-5 md:p-6 space-y-6 overflow-y-auto no-scrollbar flex-1">
                    <div className="space-y-6">
                        {/* Status selector (Solo en edici├│n o siempre visible) */}
                        {isEditing && (
                        ...
                        <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-950/50 rounded-2xl border border-slate-200 dark:border-white/10">
                            <div>
                                ...
                                <p className="text-xs font-bold text-slate-700 dark:text-slate-300">
                                    {formData.isPrivate ? 'Privada (Solo yo)' : 'P├║blica (Administradores y Asignados)'}
                                </p>
                            </div>
                            <button
                                type="button"
                            ...
                            >
                                <div className={cn(
                                    ...
                                    <span className="text-[10px] font-black uppercase">{formData.isPrivate ? 'Privada' : 'P├║blica'}</span>
                                </div>
                                <div className={cn("w-1/2 text-center text-[10px] font-black uppercase", !formData.isPrivate ? "" : "hidden")}>P├║blica</div>
                                <div className={cn("w-1/2 text-center text-[10px] font-black uppercase ml-auto", formData.isPrivate ? "" : "hidden")}>Privada</div>
                            </button>
                        </div>

                        <div>
                            <label className="block text-[9px] font-black uppercase tracking-[0.2em] text-blue-500 mb-2">T├¡tulo de la Tarea</label>
                            <input
                                name="title"
                        ...
                        <div>
                            <label className="block text-[9px] font-black uppercase tracking-[0.2em] text-blue-500 mb-2">Descripci├│n</label>
                            <textarea
                                name="description"
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            {/* L├│gica de Fechas seg├║n Recurrencia */}
                            {formData.recurrence !== 'NINGUNA' ? (
                                <>
                                    ...
                                    <div className="relative" ref={datePickerRef}>
                                        <label className="block text-[9px] font-black uppercase tracking-[0.2em] text-blue-500 mb-2">Hasta (L├¡mite)</label>
                                        <div
                            ...
                            <div className="flex items-center justify-between p-4 bg-green-50 dark:bg-green-900/10 rounded-2xl border border-green-200 dark:border-green-900/20">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-500 font-medium">Enviar mensaje a t├®cnicos por cada tarea generada</p>
                                    </div>
                                </div>
                                <button
                                    type="button"
                        ...
                        <div className="p-4 bg-blue-500/5 dark:bg-blue-500/10 rounded-2xl border border-blue-500/20">
                            ...
                            {formData.reminder_at && (
                                <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                                    ...
                                    <p className="text-[9px] text-slate-500 font-bold italic">* Recibir├ís una alerta en el aplicativo a esta hora.</p>
                                </div>
                            )}
                        </div>

                        <div>
                            <label className="block text-[9px] font-black uppercase tracking-[0.2em] text-blue-500 mb-2">T├®cnicos Asignados</label>
                            <div className="relative" ref={techSearchRef}>
                                <div className="relative flex items-center mb-3">
                                    ...
                                    <input
                                        ...
                                        placeholder="Buscar t├®cnico de GLPI..."
                                    />
                                ...
                                {isTechListOpen && canEditFull && (
                                    <div className="absolute top-full left-0 w-full z-[70] bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl p-2 max-h-60 overflow-y-auto mt-2 animate-in slide-in-from-top-2 no-scrollbar custom-scrollbar shadow-blue-500/5">
                                        {filteredTechs.length === 0 ? (
                                            <div className="p-4 text-center text-slate-500 text-xs font-bold">No se encontraron t├®cnicos</div>
                                        ) : (
                                            filteredTechs.map(t => (
                                                <div
                                                    key={t.id}
            ...
            {/* Modal de confirmaci├│n de eliminaci├│n con estilo Premium */}
            {showDeleteConfirm && (
                <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2rem] border border-slate-200 dark:border-white/10 shadow-2xl p-6 animate-in zoom-in-95 duration-200">
                        ...
                        <p className="text-center text-slate-500 text-sm mb-6 font-medium">
                            Esta acci├│n no se puede deshacer. La tarea ser├í eliminada permanentemente.
                        </p>
