...
// Control de concurrencia para evitar picos de memoria (máximo 2 browsers simultáneos)
const PDF_CONCURRENCY_LIMIT = 2;
let activePdfProcesses = 0;

const waitForSlot = async () => {
  while (activePdfProcesses >= PDF_CONCURRENCY_LIMIT) {
...
export const generateMaintenancePDF = async (actData) => {
  ...
  try {
    ...
    console.error('[PDF] Error generando PDF de mantenimiento:', error.message);
    throw error;
  } finally {
    releaseSlot();
...
export const generateConsolidatedPDF = async (clientName, acts) => {
  ...
  try {
    ...
    console.error('[PDF] Error generando PDF consolidado:', error.message);
    throw error;
  } finally {
    releaseSlot();
    if (browser) await browser.close();
...
export const generateQuotationPDF = async (q) => {
  ...
  try {
    ...
    console.error('[PDF] Error generando PDF de cotización:', error.message);
    throw error;
  } finally {
    releaseSlot();
    if (browser) await browser.close();
