                ...
                <div className="flex items-center gap-4">
                    ...
                    {canCreate && (
                        <button
                            ...
                            className="bg-primary-500 hover:bg-primary-600 text-white px-8 py-3.5 rounded-2xl text-[11px] font-black tracking-[0.1em] shadow-lg transition-all flex items-center gap-3 active:scale-95 uppercase group"
                        >
            ...
            <div className="bg-secondary border border-color rounded-[12px] shadow-sm flex flex-col overflow-hidden transition-colors duration-300">
                ...
                <div className="relative min-h-[400px]">
                    {viewMode === 'calendar' ? (
                        <>
                            {/* Desktop Grid View */}
                            <div className="hidden lg:grid grid-cols-7 border-b border-color bg-primary transition-colors">
                                {daysArr.map(d => (
                                    <div key={d} className="py-2.5 text-center text-[11px] font-[600] uppercase text-text-muted tracking-[1px]">{d}</div>
                                ))}
                            </div>

                            <div className="hidden lg:grid grid-cols-7 flex-1 overflow-auto no-scrollbar">
                                {calendarDays.map((date, i) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={cn(
                                                "min-h-[140px] flex flex-col p-2 border-r border-tertiary border-b border-tertiary transition-all relative group/day",
                                            ...
                                            {date && (
                                                <>
                                                    <div className="flex items-center justify-between mb-2">
                                                        <span className={cn(
                                                            "text-[12px] font-[800] flex items-center justify-center w-[24px] h-[24px] rounded-full",
                                                    ...
                                                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-1 max-h-[100px] pr-1">
                                                        {taskList.sort((a, b) => new Date(a.scheduled_at) - new Date(b.scheduled_at)).map(task => (
                                                            <div
                                                                ...
                                                                className={cn(
                                                                    "px-2 py-1.5 rounded-lg bg-tertiary text-[10px] font-[700] truncate border border-color cursor-pointer hover:bg-secondary hover:shadow-sm transition-all",
                            ...
                            {/* Mobile List View */}
                            <div className="lg:hidden flex flex-col divide-y divide-color">
                                {calendarDays.filter(d => d).map((date, i) => {
                                    const taskList = filteredTasks.filter(t => t.scheduled_at && new Date(t.scheduled_at).toDateString() === date.toDateString());
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2">
                                                    <span className={cn(
                                                        ...
                                                        isToday ? "bg-primary-500 text-white shadow-lg" : "bg-tertiary text-text-primary"
                                                    )}>
                                                        {date.getDate()}
                                                    </span>
                                                ...
                                                <button
                                                    ...
                                                    className="p-2 bg-tertiary border border-color rounded-xl text-primary-500"
                                                >
                                                    <Plus size={16} />
                                                </button>
                                            </div>
                                            <div className="space-y-2">
                                                {taskList.map(task => (
                                                    <div
                                                        ...
                                                        className="p-4 rounded-2xl bg-tertiary border border-color active:scale-[0.98] transition-all"
                                                    >
                                                        <div className="flex items-center gap-2 mb-1.5">
                                                            <div className={cn("w-2 h-2 rounded-full", task.priority === 'CRITICA' ? 'bg-red-500' : 'bg-primary-500')} />
                                                        ...
                                                        <h4 className="text-sm font-black text-text-primary">{task.title}</h4>
                                                        <p className="text-xs text-text-muted mt-1.5 line-clamp-1">{task.description}</p>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                ...
                                {calendarDays.filter(d => d && filteredTasks.some(t => t.scheduled_at && new Date(t.scheduled_at).toDateString() === d.toDateString())).length === 0 && (
                                    <div className="p-20 text-center">
                                        <div className="w-16 h-16 bg-tertiary rounded-full flex items-center justify-center mx-auto mb-4 opacity-30">
                                            <CalendarIcon size={32} />
                                        </div>
                                        <p className="text-[11px] font-black uppercase tracking-widest text-text-muted">No hay tareas este mes</p>
                                    </div>
                                )}
                            </div>
                        </>
