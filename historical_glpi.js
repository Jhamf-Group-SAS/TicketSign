...
class GLPIConnector {
    ...
    /**
     * Inicializa la sesi├│n en GLPI
     */
    async initSession() {
        const { apiUrl, appToken, userToken } = this.config;
        console.log(`[GLPI] Iniciando sesi├│n en: ${apiUrl}`);
        ...
        try {
            ...
            console.log(`[GLPI] Sesi├│n establecida. ID Sesi├│n: ${this.sessionToken?.substring(0, 10)}...`);
            ...
            if (!Array.isArray(profiles)) {
                // Si es un objeto, lo ponemos en un array. Si es null/undefined, array vac├¡o.
            ...
            if (isAlreadyAllowed) {
                ...
                if (targetProfile && targetProfile.id !== activeProfileId) {
                    ...
                    console.log(`[GLPI] No se encontr├│ un perfil mejor. Operando con: ${activeProfileName}`);
                }
            }
    ...
    /**
     * Sube un documento y lo asocia a un ├¡tem (Ticket o Project)
     */
    async uploadDocument(itemId, filePath, fileName, itemtype = 'Ticket') {
        ...
        try {
            // 0. Diagn├│stico previo (solo para Tickets)
            if (false && itemtype === 'Ticket') {
                try {
                    ...
                    console.log(`[GLPI] Diagn├│stico Ticket #${itemId}: Estado=${ticket.status}, Entidad=${ticket.entities_id}`);
    ...
    /**
     * Agrega un seguimiento al ├¡tem
     */
    async addFollowup(itemId, content, itemtype = 'Ticket') {
        if (!this.sessionToken) await this.initSession();
        const { apiUrl, appToken } = this.config;

        // GLPI usa ITILFollowup para Tickets, pero para Projects podr├¡a variar. 
        ...
        try {
            ...
            console.log(`[GLPI] Seguimiento a├▒adido al ${itemtype} #${itemId}`);
        } catch (error) {
    ...
    /**
     * Obtiene t├®cnicos elegibles basados en sus perfiles
     */
    async getEligibleTechnicians() {
        ...
        try {
            console.log('[GLPI] Buscando t├®cnicos mediante Profile_User...');
            ...
            if (!Array.isArray(response.data)) {
                // Si la respuesta no es un array directo, puede que est├® envuelta (v1 de la API antigua)
                const data = Array.isArray(response.data) ? response.data : (response.data.data || []);
                if (!Array.isArray(data)) {
                    console.error('[GLPI] Error: /Profile_User no devolvi├│ un array');
                    return [];
                }
                response.data = data;
            }
            ...
            for (const entry of response.data) {
                ...
                // Fallback al "id" si es n├║mero (en algunas versiones este es el user id si viene filtrado)
            ...
            console.log(`[GLPI] Identificados ${eligibleUsers.length} t├®cnicos por perfil. Obteniendo detalles adicionales (m├│vil)...`);

            // Obtener detalles (especialmente el m├│vil) para cada t├®cnico encontrado
            ...
            for (let i = 0; i < eligibleUsers.length; i += BATCH_SIZE) {
                ...
                await Promise.all(batch.map(async (tech) => {
                    try {
                        ...
                        // GLPI suele guardar el m├│vil en mobile, phone, o phone2. Probamos mobile primero.
                        tech.mobile = userData.mobile || userData.phone || '';
                    } catch (err) {
                        console.warn(`[GLPI] No se pudo obtener detalle para t├®cnico ${tech.id}`);
                    }
                }));
            }

            console.log(`[GLPI] B├║squeda finalizada. ${eligibleUsers.length} t├®cnicos listos.`);
    ...
    /**
     * Obtiene tickets (filtro b├ísico)
     */
    /**
     * Obtiene tickets (filtro b├ísico)
     */
    async getTickets(criteria = {}) {
        ...
        try {
            ...
            // 2. Mis Tickets (Filtro inclusivo: T├®cnico o Solicitante)
            ...
            if (techFilterId && techFilterId !== 'undefined' && techFilterId !== 'null' && techFilterId !== 'all') {
                ...
                // Intentar obtener el nombre del t├®cnico de nuestro cache para un fallback por nombre
                ...
                tickets = tickets.filter(t => {
                    ...
                    // Fallback por nombre (importante si expand_dropdowns nos quit├│ el ID en el objeto principal)
                    ...
                    // Tambi├®n buscamos en los arrays de IDs originales por si acaso
            ...
            console.log(`[DEBUG-SRV] Finalizado getTickets. Retornando ${tickets.length}/${response.data.length} items. Filtro t├®cnico: ${techFilterId || 'no'}`);
            return tickets;
    ...
    async getTicket(id) {
        ...
        try {
            ...
            try {
                ...
                console.warn(`[GLPI] Ticket_User NO disponible mediante ruta anidada. Intentando b├║squeda alternativa...`);
                try {
                    // Intento alternativo via b├║squeda filtrada
                    const aResAlt = await axios.get(`${apiUrl}/Ticket_User`, {
            ...
            try {
                ...
                console.warn(`[GLPI] Ticket_Group NO disponible mediante ruta anidada. Intentando b├║squeda alternativa...`);
                try {
                    const gaResAlt = await axios.get(`${apiUrl}/Ticket_Group`, {
                        params: { searchText: { tickets_id: id } },
                        headers
            ...
            // 5. Normalizaci├│n b├ísica inicial para enriquecimiento
            ...
            await Promise.all(Array.from(userIdsToFetch).map(async (uid) => {
                ...
                if (!isStored) {
                    try {
                        ...
                        // Soporte para formato num├®rico de GLPI (1=login, 9=firstname, 34=realname)
            ...
            const getCachedName = (val) => {
                ...
                if (typeof val === 'object') {
                    // Soporte para formato est├índar y formato num├®rico (1=login, 9=firstname, 34=realname)
                    ...
                    if (nameFromObj) {
                        // Si tenemos el objeto, tambi├®n lo guardamos en cache para futuras referencias
            ...
            const getActorName = (a, isGroup) => {
                // 1. Intentar obtener nombre directo si ya viene en el objeto (ej: b├║squeda expandida)
                const directName = a.completename || a.name || a.fullName || a.groups_id_name || a.users_id_name ||
                    (a["9"] ? (a["34"] ? `${a["9"]} ${a["34"]}` : a["9"]) : null);

                if (directName && typeof directName === 'string' && directName.length > 0) return directName;

                // 2. Si no hay nombre, buscar ID para consultar el cach├®
                const id = (isGroup ? a.groups_id : a.users_id) || a.id || a["2"] || a["5"];
            ...
            // Fallback a campos del ticket si no hay actores expl├¡citos
    ...
    async addSolution(ticketId, content) {
        ...
        try {
            console.log(`[GLPI] Creando soluci├│n para Ticket #${ticketId}`);
            // Tipo de soluci├│n por defecto (puede requerir ajuste seg├║n configuraci├│n GLPI)
    ...
    async getUsers() {
        ...
        try {
            ...
            const result = data.map(u => {
                // Soporte para formato est├índar y formato num├®rico
    ...
    /**
     * Obtiene una lista gen├®rica de GLPI (Categor├¡as, Ubicaciones, etc)
     */
    ...
    /**
     * Obtiene grupos (para asignaci├│n)
     */
    async getGroups() {
        if (!this.sessionToken) await this.initSession();
        const { apiUrl, appToken } = this.config;
    ...
    /**
     * Actualiza o a├▒ade un actor a un ticket
     */
